    ...
    match command {
        ...
        ClobCommands::GetOrderBooks { token_id, side } => {
            let parsed_side = side
                .as_ref()
                .map(|s| s.parse::<polymarket_hft::client::clob::Side>())
                .transpose()
                .map_err(|e| anyhow::anyhow!("invalid --side: {}", e))?;

            let request: Vec<polymarket_hft::client::clob::GetOrderBooksRequestItem> = token_id
                .iter()
                .map(
                    |id| polymarket_hft::client::clob::GetOrderBooksRequestItem {
                        token_id: id.clone(),
                        side: parsed_side,
                    },
                )
        ...
        ClobCommands::GetMarketPrice { token_id, side } => {
            let parsed_side = side
                .parse::<polymarket_hft::client::clob::Side>()
                .map_err(|e| anyhow::anyhow!("invalid --side: {}", e))?;
        ...
        ClobCommands::GetPriceHistory { params } => {
            let parsed_interval = params
                .interval
                .as_ref()
                .map(|s| s.parse::<polymarket_hft::client::clob::PriceHistoryInterval>())
                .transpose()
                .map_err(|e| anyhow::anyhow!("invalid --interval: {}", e))?;

            let history = client
                .get_price_history(polymarket_hft::client::clob::GetPriceHistoryRequest {
        ...
        ClobCommands::GetSpreads { token_id } => {
            let request: Vec<polymarket_hft::client::clob::SpreadRequest> = token_id
                .iter()
                .map(|id| polymarket_hft::client::clob::SpreadRequest {
                    token_id: id.clone(),
                    side: None,
                })
                .collect();
